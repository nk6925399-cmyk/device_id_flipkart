...
# Connect to MongoDB if MONGO_URI environment variable is provided
...
if MONGO_URI:
    try:
        import pymongo
        # Establish connection with a 5-second timeout for server selection
        client = pymongo.MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000)
        db = client['device_auth_db']
        ...
        sys.stderr.write("❌ pymongo package is not installed. To use MongoDB, run: pip install pymongo dnspython\n")
        sys.stderr.write("⚠️ Falling back to local devices.json file.\n")
        sys.stderr.flush()
        USE_MONGO = False
    ...
    print("⚠️ MONGO_URI environment variable not found. Using local devices.json.")
...
class DashboardHandler(http.server.BaseHTTPRequestHandler):
    ...
    def load_devices(self):
        if USE_MONGO:
            try:
                ...
                sys.stderr.write(f"MongoDB load error: {str(e)}\n")
                sys.stderr.flush()
                return {}
        else:
            try:
    ...
    def save_device_to_db(self, device_id, device_info):
        if USE_MONGO:
            try:
                ...
                sys.stderr.write(f"MongoDB write error: {str(e)}\n")
                sys.stderr.flush()
                return False
        else:
    ...
    def delete_device_from_db(self, device_id):
        if USE_MONGO:
            try:
                devices_collection.delete_one({'_id': device_id})
                return True
            except Exception as e:
                sys.stderr.write(f"MongoDB delete error: {str(e)}\n")
                sys.stderr.flush()
                return False
        else:
            data = self.load_devices()
...
<body>
    ...
    def do_GET(self):
        ...
        if path == '/validate.php' or path == '/validate':
            ...
            if USE_MONGO:
                try:
                    device_info = devices_collection.find_one({'_id': device_id})
                except Exception as e:
                    sys.stderr.write(f"MongoDB validation query error: {str(e)}\n")
                    sys.stderr.flush()
                    device_info = None
            else:
                devices_data = self.load_devices()
                device_info = devices_data.get(device_id)
    ...
    def do_POST(self):
        try:
            ...
            if path == '/register_device':
                ...
                self.save_device_to_db(device_id, device_info)
            ...
            if path == '/delete_device':
                ...
                device_id = params.get('device_id', [''])[0].strip()
                self.delete_device_from_db(device_id)
